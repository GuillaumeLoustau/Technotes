<?PHP
	...
	if ($id_techquestion!="")
	{
		...
		?>
		<h2>Technote - <?PHP echo $techquestion->getTitre(); ?></h2>
		<div id ="technote">
		  <form name="fAjCommentaireTechnote" action="index.php" method="post">
			<input type="hidden" name="action" value="technoteCommentaireEnregistrer" />
			...
			<div id="commentairePere0" style="display:none;text-align:center;">
				...
				<input type="button" name="ok0" value="Envoyer" onclick="javascript : if(document.fAjCommentaireTechnote.contenuCommentaire0.value=='') alert('Veuillez saisir un commentaire !!'); else{document.fAjCommentaireTechnote.id_commentaire_pere.value=0; document.fAjCommentaireTechnote.submit();}">
					...
					<!-- <div class ="commentaire">-->
						<p>
							...
							<div id="commentairePere<?PHP echo $donnees_com['id_commentaire']; ?>" style="display:none;text-align:center;">
								...
								<input type="button" name="ok<?PHP echo $donnees_com['id_commentaire']; ?>" value="Envoyer" onclick="javascript : if(document.fAjCommentaireTechnote.contenuCommentaire<?PHP echo $donnees_com['id_commentaire']; ?>.value=='') alert('Veuillez saisir un commentaire !!'); else{document.fAjCommentaireTechnote.id_commentaire_pere.value=<?PHP echo $donnees_com['id_commentaire']; ?>; document.fAjCommentaireTechnote.submit();}">
