<h2>Questions</h2>
<?PHP
if ($_SESSION['questionConsulter'] == 1)
{
...
?>
	<form name="frecherche" action="index.php" method="post">
		<input type="hidden" name="action" value="questionsListe"/>
		<span class="recherche">
			...
			Pos&eacute;e entre <input type ="text" name = "date1Req" value = "<?PHP if(isset($_POST['date1Req'])) {echo $_POST['date1Req'];} ?>"> et le <input type ="text" name = "date2Req" value = "<?PHP if(isset($_POST['date1Req'])) {echo $_POST['date2Req'];} ?>">
			...
			Statut <select name="statutReq" class="selectRecherche">
				<option value="1" <?PHP if(!isset($_POST['statutReq']) || $_POST['statutReq']== '') {echo 'selected';} ?>>Toutes</option>
				<option value="1" <?PHP if(isset($_POST['statutReq']) && $_POST['statutReq']== '1') {echo 'selected';} ?>>R&eacute;solues</option>
				<option value="0" <?PHP if(isset($_POST['statutReq']) && $_POST['statutReq']== '0') {echo 'selected';} ?>>Non r&eacute;solues</option>
			</select>
	...
	<table width="100%" cellpadding="3">
		<tr>
			<td valign="top" colspan="3">
				<?PHP 
				if ($_SESSION['questionAjouter'] == 1) echo "<a href=\"index.php?action=questionFormulaire&id_techquestion=\" class=\"technoteLien\">Poser une question</a>";
				?>
			</td>
		</tr>
		<tr>
			...
			<td valign="top"><span class="titreColonne">Questions - <i>mot cles</i></span></td>
		</tr>
		...
		<?PHP
			...
			$listeTechquestions = $techquestionGestion->getListRecherche(2, $ordre_tmp, $direction_tmp, $id_auteur_tmp, $date1_tmp, $date2_tmp, $statut_tmp, $motscles_tmp);
			if ($listeTechquestions) 
			{
				...
				?>
					<tr>
						...
						<td valign="top">
							<a class="contenuCellule" href ="index.php?action=questionConsultation&id_techquestion=<?PHP echo $id_techquestion; ?>"><?PHP echo $donnees['titre']; ?></a>
							<br/><span class="motClesListe"><?PHP echo $donnees['motcles']; ?></span>
							<?PHP if ( ( $_SESSION['questionModifier'] == 1) || ( $_SESSION['questionAjouter'] == 1 && $_SESSION['id_membre'] == $donnees['id_membre'] ) ) { ?><a class="lienMenu" href ="index.php?action=questionFormulaire&id_techquestion=<?PHP echo $id_techquestion; ?>">modifier</a> <?PHP }?>
						</td>
					</tr>
					<?PHP
				}
